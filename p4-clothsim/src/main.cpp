...
const string CTRIANGLE = "ctriangle";

const unordered_set<string> VALID_KEYS = {SPHERE, PLANE, CLOTH, CTRIANGLE};
...
void createGLContexts() {
    ...
    window = glfwCreateWindow(800, 800, "Cel Shader", nullptr, nullptr);
    if (window == nullptr) {
...
bool loadObjectsFromFile(string filename, Cloth *cloth, ClothParameters *cp, vector<CollisionObject *>* objects, int sphere_num_lat, int sphere_num_lon) {
    ...
    for (json::iterator it = j.begin(); it != j.end(); ++it) {
        ...
        if (key == CLOTH) {
            ...
            objects->push_back(s);
        } else if (key == CTRIANGLE) {
            Vector3D pointA, pointB, pointC, normal;
            double friction;
            
            auto it_pA = object.find("pointA");
            if (it_pA != object.end()) {
                vector<double> vec_pA = *it_pA;
                pointA = Vector3D(vec_pA[0], vec_pA[1], vec_pA[2]);
            } else {
                incompleteObjectError("ctriangle", "pointA");
            }
            
            auto it_pB = object.find("pointB");
            if (it_pB != object.end()) {
                vector<double> vec_pB = *it_pB;
                pointB = Vector3D(vec_pB[0], vec_pB[1], vec_pB[2]);
            } else {
                incompleteObjectError("ctriangle", "pointB");
            }
            
            auto it_pC = object.find("pointC");
            if (it_pC != object.end()) {
                vector<double> vec_pC = *it_pC;
                pointC = Vector3D(vec_pC[0], vec_pC[1], vec_pC[2]);
            } else {
                incompleteObjectError("ctriangle", "pointC");
            }
            
            auto it_n = object.find("normal");
            if (it_n != object.end()) {
                vector<double> vec_n = *it_n;
                normal = Vector3D(vec_n[0], vec_n[1], vec_n[2]);
            } else {
                incompleteObjectError("ctriangle", "normal");
            }
            ...
            if (it_friction != object.end()) {
                ...
                incompleteObjectError("ctriangle", "friction");
            }
            cTriangle *t = new cTriangle(pointA, pointB, pointC, normal, friction);
            objects->push_back(t);
        } else { // PLANE
